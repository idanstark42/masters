...
Identities:
\begin{equation*}
    F^{0i} = F_{i0} = -F_{0i} = -F^{i0}
    \qquad
    F^{ij} = -F^{ji} = F_{ij} = -F_{ji}
\end{equation*}
...
\begin{gathered}
    ...
    \left(2\pi\right)^3\delta^3(\vec{k} - \vec{p})
    \\
    \left[a_{\vec{k}}, a_{\vec{p}}\right] =
...
\subsection{Lagrangian formulation (fields)}
Classical fields \(\phi_a(x)\), \(x^\mu = (t,\vec{x})\).

\begin{equation*}
    \mathcal{L} = \mathcal{L}\!\left(\phi_a,\partial_\mu\phi_a\right)
\end{equation*}
...
\begin{equation*}
    \dot\phi_a = \frac{\delta H}{\delta \pi_a}
    \qquad
    \dot\pi_a = -\frac{\delta H}{\delta \phi_a}
\end{equation*}
\end{document}