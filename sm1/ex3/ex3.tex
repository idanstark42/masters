...
\newline
% After reaching convergence, the simultion ran over the twenty samples for an additional 1000 sweeps, using these 20,000 samples (20 samples over 1000 sweeps) as the population for the distribution. This distribution can be seen in figure \ref{fig:direct-sampling}.
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.7\textwidth]{monte_carlo_histogram_20260104_021317.png}
...
\end{equation}
% with $k = 2000$\footnote{The question required $k = 200$, but after a trial it did not move the center to $0.6$, only to $0.4$. This is due to the bias not being strong enough against the original hamiltonian, the entropy and the high temperatuer pushing the magnetization to the center a non-negligble amount. To bypass this and see the full effect of the Umbrella potential, we used a bias one order of magnitude higher.} and $m_0$ = 0.6. The new distribution can be seen in figure \ref{fig:umbrella-potential}. It can be seen that the new center of the distribution is $m = 0.6$.
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.7\textwidth]{monte_carlo_histogram_20260104_021839.png}
...
\end{equation}
% This allows us to evaluate $P(m)$ using a modified hamiltonian, not having the same numerical sampling biases as the original hamiltonian. In our case, we can remove the bias we gained in the previous subsection. The new distribution after removing the bias can be found in figure \ref{fig:umbrella-potential-after-correction}. It can be seen that this time the distribution is not a full gaussian, but rather only the right tail. To see that it matches figure \ref{fig:direct-sampling}, let us normalize both distributions and plot them together, as can be seen in figure \ref{fig:direct-vs-umbrella}. 
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.7\textwidth]{unbiased_magnetization_histogram_20260104_023051.png}
...
\begin{equation}
    \rho^2 + \frac{4\tau^2 - 2 + 2\alpha_k^2}{1 + \alpha_k}\rho + 1
     = 0
\end{equation}
And so we got a quadratic equation for $\rho$. Solving it will give the values of $\rho$ for which $Z = 0$. Figure \ref{fig:zeros-of-z-different-n} shows the zeros on the complex plain for $N = 10, 50, 500$ and $\tau = 0.5$.
\begin{figure}[ht]
  \centering
